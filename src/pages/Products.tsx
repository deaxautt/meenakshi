...
const Products = () => {
  ...
  const filters = {
    categories: [
      { name: 'Gemstones', count: products.filter(p => p.category === 'Gemstones').length },
      { name: 'Rudraksha', count: products.filter(p => p.category === 'Rudraksha').length },
      { name: 'Traditional Wear', count: products.filter(p => p.category === 'Traditional Wear').length }
    ],
    priceRanges: [
    ...
    if (selectedFilters.length > 0) {
      result = result.filter(product => 
        selectedFilters.some(filter => 
          ...
          product.rashi?.includes(filter) ||
          (filter.includes('%') && parseFloat(product.discount) >= parseFloat(filter))
        )
      );
    }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className={`w-full md:w-64 space-y-6 ${showFilters ? 'block' : 'hidden md:block'}`}>
            ...
            {/* Rashi Filter */}
            <div className="bg-white rounded-lg">
              <h3 className="font-semibold mb-4">Rashi (Zodiac)</h3>
              <div className="space-y-2">
                {filters.rashi.map((rashi, index) => (
                  <label key={index} className="flex items-center gap-2 cursor-pointer group">
                    <input 
                      type="checkbox"
                      checked={selectedFilters.includes(rashi)}
                      onChange={() => toggleFilter(rashi)}
                      className="rounded border-gray-300 text-primary focus:ring-primary"
                    />
                    <span className="text-gray-700 group-hover:text-primary transition-colors">
                      {rashi}
